...
if (!file.exists("polak_count_overlap_data/processed_count_overlaps/polak_combined_count_overlaps.rds")) {
  polak_combined_count_overlaps = data.frame()
  for (file in list.files("polak_count_overlap_data", full.names=TRUE, pattern=
                          "count_overlaps_")) {
    if (grepl("uniques", file)) {
      next
    }
    else {
      tissue_or_cell_name = unlist(strsplit(file, ".", fixed = TRUE))[2]
    ...
    tryCatch(
          if (count_overlaps_exists) {
            ...
            polak_combined_count_overlaps[idx, ] = polak_combined_count_overlaps[idx, ] +
              count_overlaps[1, ]
          }
          else {
...
correlations = cor(polak_combined_count_overlaps, mut_count_data, use="complete.obs")
