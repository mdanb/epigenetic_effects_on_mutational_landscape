...
for i in "${!cancer_types[@]}"; do
    ...
    figure1="${root}${cancer_types[$i]}_top_feature_appearances.svg"
    figure2="${root}${cancer_types[$i]}_top_feature_test_set_perf_with_10_5_2_1_features.svg"
    output_svg="${root}${cancer_types[$i]}_side_by_side.svg"
    ...
    inkscape --export-filename="$output_svg" "$figure1" "$figure2"
    #side_by_side_pdfs+=("$output_pdf")
done

#inkscape --export-filename="combined.svg" "${side_by_side_svgs[@]}"
