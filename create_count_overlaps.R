...
for (file in setdiff(list.files("raw_dir/bed_files/"), list.dirs("raw_dir/bed_files", 
	...
	if (!file.exists(filename)) {                                           
                ...
                sample_name = str_extract(file, pattern="_.*_rep")              
                sample_name = substr(sample_name, 2, nchar(sample_name) - 4)    
                ...
                cell_type_keep = counts_per_cell_type[counts_per_cell_type$n_cells >= 10, ]$cell_type
                sample = sample %>%                                             
