...
parser <- add_option(parser, c("--subsampled_mutations"), action="store_true",
                     default=F)
parser <- add_option(parser, c("--fig1b"), action="store_true",
                     default=F)
...
# args = parse_args(parser, args= c("--cancer_types=Lymph-BNHL,Bone-Leiomyo,Thy-AdenoCA,Uterus-AdenoCA",
#                                   "--annotation=finalized_annotation",
#                                   "--ML_model=XGB",
#                                   "--seed_range=1-10",
#                                   "--folds_for_test_set=1-10",
...
#                                   "--grid_cell_types=bonemarrow B GL_BlBm,stomach Stromal cells SH,thyroid Thyroid Follicular Cell BR,placenta PAEP_MECOM positive cells SH"))

# Breast-AdenoCa
# Myeloid-AML
# mammary_tissue Basal Epithelial (Mammary) BR, bonemarrow Early.Baso GL_BlBm
...
#                                  "--top_features_to_plot=10,5,2,1"))
# args = parse_args(parser, args= c("--cancer_types=CNS-Medullo,Panc-AdenoCA",
#                                   "--cell_number_filter=100",
#                                   "--datasets=Bingren,Bingren_adult_brain,Greenleaf_brain,Shendure-Bingren,Shendure",
#                                   "--tissues_to_consider=adult_brain,brain,frontal_cortex,cerebrum,cerebellum-pancreas,stomach",
#                                   "--ML_model=XGB",
#                                   "--seed_range=1-10",
#                                   "--feat_imp_min_n_robustness=50",
#                                   "--folds_for_test_set=1-10",
...
cell_types = c("brain OPC/Oligo GL_Br" = "OPC/Oligo, Cerebral Cortex GL_Br",
               ...
               "fetal_lung Ciliated R_Fl" = "Ciliated, Fetal Lung R_Fl",
...
rename_cell_types <- function(cell_type_names) {
  ...
  renamed = gsub("BR", "D1", renamed) 
  renamed = gsub("SH", "D2", renamed) 
  renamed = gsub("GL_Co", "D3", renamed) 
  renamed = gsub("GL_BlBm", "D4", renamed) 
  renamed = gsub("TS", "D5", renamed) 
  renamed = gsub("Y_K", "D6", renamed) 
...
if (!robustness_analysis) {
  ...
  #                        c(1,5,10,15,20,25,30,35),
  #                        c(1,5,10,15,20,25,30,35,40,45))
  cancer_type_n = list(c(1,5,10,15,20,25,30,35),
                       c(1,5,10,15,20,25,30,35),
                       c(1,5,10,15,20,25,30,35),
                       c(1,5,10,15,20,25,30,35),
                       c(1,5,10,15,20,25,30,35),
  ...
  for (cancer_type in cancer_types) {
    ...
    if (!is.na(cell_types_keep[i])) {
      scATAC_source = paste(scATAC_source, "ctk", cell_types_keep[i], sep="_")
    }
...
if (subsampled_mutations) {
  cols =  paletteer::paletteer_d("ggprism::colorblind_safe") 
  ...
  custom_order = c("CNS-Medullo", "Kidney-ChRCC", "Liver-HCC", 
                   "CNS-GBM", "Lung-SCC", "Skin-Melanoma")
